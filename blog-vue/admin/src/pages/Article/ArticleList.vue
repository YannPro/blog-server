        ...
        <el-table-column label="操作" width="285">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              type="info"
              icon="setting"
              @click="handleRoleConfig(scope.$index, scope.row)">预览
            </el-button>
            <el-button
              size="small"
              type="danger"
...
<script>
  ...
  export default {
    ...
    mounted(){
      ...
      //       console.log(item)
      //       return item.createTime = formatDate(new Date(item.createTime), "yyyy-MM-dd h:m:s")
      //     })
    ...
    methods: {
      ...
      loadData(){
        let param = {}
        param.limit = this.tableData.pagination.pageSize
        param.page = this.tableData.pagination.pageNo
        getArticleList(param).then(res=>{
          if(res.status === 1){
            this.tableData.rows = res.data.rows.filter(item => {
              item.displayClass = ""
              for(let o of item.classList){
                item.displayClass += o.className + ","
              }
              item.displayClass = item.displayClass.substring(0, item.displayClass.length-1)
              return item.createTime = formatDate(new Date(item.createTime), "yyyy-MM-dd h:m:s")
            })
